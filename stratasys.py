# From https://github.com/nkaminski/stratasys_api/
import re
import json
from struct import *
import socket
...
def stratasys_out_proc(stra):
    ...
    for s in stra:
        ...
        if (p == None):
            ...
            if len(out_dict) > 0:
                if out_dict['machineStatus(general)']['modelerType'] == 'waikiki' and p.group('category')=='machineStatus(cassette)':

                    tmp2=re.split(r'{\t', p.string)
                    for i in range(4):
                        tmp3=bracesplit.search("set machineStatus(cassette)"+" {{\t"+tmp2[i+1]+"}")
                        out_dict["set machineStatus(cassette"+str(i)+")"] = objproc(tmp3.group('category'), tmp3.group('value'))
                else:
